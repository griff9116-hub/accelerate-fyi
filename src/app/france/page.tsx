...
export const metadata: Metadata = {
  title: "French Startup Accelerators & VCs (2025)",
  description: "The best startup accelerators, venture studios, and investors in France — Paris, Lyon, and Station F.",
};

export default async function FrancePage() {
  const rows = await prisma.programme.findMany({
    where: { isActive: true, country: "France", NOT: { type: "VC" } },
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
      <div className="mb-10">
        <p className="mb-2 text-sm text-zinc-500"><Link href="/europe" className="hover:text-zinc-300">Europe</Link> / France</p>
        <h1 className="text-4xl font-bold text-white">French Startup Accelerators &amp; Investors</h1>
        <p className="mt-3 text-zinc-400">{programmes.length} programmes in France.</p>
      </div>

      <AdSlot slot="category-france" format="horizontal" className="mb-6" />

      {programmes.length === 0 ? (
        <div className="rounded-xl border border-dashed border-zinc-800 p-16 text-center">
          <p className="text-zinc-500">French programmes coming soon.</p>
